import { Address, CurrencyISO, FundsType, Money } from '../type';
import type { Api } from '../mangopay';

type BusinessType = 'MARKETPLACE' | 'CROWDFUNDING' | 'FRANCHISE' | 'OTHER';
type Sector = 'RENTALS' | 'STORES_FASHION_ACCESSORIES_OBJECTS' | 'BEAUTY_COSMETICS_HEALTH' | 'FOOD_WINE_RESTAURANTS' | 'HOSPITALITY_TRAVEL_CORIDING' | 'ART_MUSIC_ENTERTAINMENT' | 'FURNITURE_GARDEN' | 'SERVICES_JOBBING_EDUCATION' | 'SPORT_RECREATION_ACTIVITIES' | 'TICKETING' | 'LOAN' | 'EQUITY' | 'PROPERTY_EQUITY' | 'REWARDS_CHARITY' | 'POOL_GROUP_PAYMENT' | 'FRANCHISE' | 'OTHER';
